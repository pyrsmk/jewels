<template>
  <div>
    <SliderControl
      ...
      :model-value="instance.options.numStates ?? 14"
      :min="4" :max="32" :step="1"
      :display-fn="v => Math.round(v)"
      @update:model-value="instance.setParameters({ numStates: Math.round($event) })"
    />

    <SliderControl
      label="Seuil"
      :model-value="instance.options.threshold ?? 1"
      :min="1" :max="8" :step="1"
      :display-fn="v => Math.round(v)"
      @update:model-value="instance.options.threshold = Math.round($event)"
    />
