...
const simRpsFS = `#version 300 es
precision highp float;
uniform sampler2D u_state;
uniform ivec2 u_gridSize;
uniform int u_numSpecies;
...
void main() {
  ...
  int species = int(raw * float(u_numSpecies - 1) + 0.5);
  // Predator is the next species in the cycle
  int predator = (species + 1) % u_numSpecies;
  float predNorm = float(predator) / float(u_numSpecies - 1);
  int predCount = 0;
  for (int dy = -1; dy <= 1; dy++) {
    for (int dx = -1; dx <= 1; dx++) {
      ...
      int nsi = int(ns * float(u_numSpecies - 1) + 0.5);
      if (nsi == predator) predCount++;
    }
  }
  // If enough predators surround us, we get consumed and become the predator
  float newState = (predCount >= u_threshold) ? predNorm : raw;
  if (newState == raw && u_spawnRate > 0.0) {
    ...
    if (r < u_spawnRate) {
      ...
      int randSpecies = int(r2 * float(u_numSpecies));
      newState = float(randSpecies) / float(u_numSpecies - 1);
    }
  }
...
export class RPSSource extends AbstractSource {

  constructor(options = {}) {
    const defaults = {
      numSpecies: 3,
      predationThreshold: 3,
      gridResolution: 256,
      speed: 0.485,
      spawnRate: 0,
      palette: 'fluo',
    ...
    super(
      { ...defaults, ...options },
      ['numSpecies', 'predationThreshold', 'gridResolution', 'speed', 'spawnRate', 'palette'],
      [],
    );
    this.gl = null;
    this._texA = null;
  ...
  setupGPU(runtime) {
    ...
    this._simProg = this._buildProgram(simRpsFS);
  ...
  _simStep() {
    ...
    gl.uniform1i(prog.locs.u_numSpecies, Math.round(+(this.options.numSpecies ?? 3)));
    gl.uniform1i(prog.locs.u_threshold, Math.round(+(this.options.predationThreshold ?? 3)));
  ...
  renderScenePass(context = {}) {
    ...
    const pal = PALETTES[this.options.palette] ?? PALETTES.fluo;
  ...
  _initGrid() {
    ...
    const w = this._gridW;
    const h = this._gridH;
    const N = Math.round(+(this.options.numSpecies ?? 3));
  ...
  setParameters(params) {
    const needsRebuild =
      ...
      params.numSpecies !== undefined && params.numSpecies !== this.options.numSpecies ||
